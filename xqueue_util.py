...
def xqueue_login():
    ...
    xqueue_login_url = urllib.parse.urljoin(settings.XQUEUE_INTERFACE['url'], urls.XqueueURLs.log_in)
...
def login(session, url, username, password):
    ...
    (success, msg) = parse_xreply(response.content.decode('utf8'))
    return success, msg
...
def post_results_to_xqueue(session, header, body):
    ...
    (success, msg) = _http_post(session, settings.XQUEUE_INTERFACE['url'] + urls.XqueueURLs.put_result, request,
